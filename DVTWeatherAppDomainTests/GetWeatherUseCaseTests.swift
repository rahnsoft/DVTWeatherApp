...
final class GetWeatherUseCaseTests: XCTestCase {
    ...
    override func setUp() {
        ...
        getWeatherUseCase = GetWeatherUseCase(mockRepo)
        cancellables = []
    }
    ...
    func testGetWeatherSuccess() {
        let weatherData = makeWeatherData()
        mockRepo.weatherToReturn = Just(weatherData)
            .setFailureType(to: SBErrors.self)
            .eraseToAnyPublisher()

        let expectation = self.expectation(description: "Weather returned")

        getWeatherUseCase.invoke(-1.2921, 36.8219)
            .sink { completion in
    ...
    func testGetWeatherFailure() {
        mockRepo.weatherToReturn = Fail(error: SBErrors.internetError)
            .eraseToAnyPublisher()

        let expectation = self.expectation(description: "Weather error returned")
